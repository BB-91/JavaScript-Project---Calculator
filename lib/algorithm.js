...
const getTokens = (equationStr) => {
    console.log(`-- DEBUG -- equationStr A: ${equationStr} --`)
    equationStr = equationStr.replaceAll(/(--|[\n\t])/g, "");
    console.log(`-- DEBUG -- equationStr B: ${equationStr} --`)

    equationStr = equationStr.replaceAll("×", "*");
    console.log(`-- DEBUG -- equationStr B2: ${equationStr} --`)

    if (equationStr.indexOf(")") < equationStr.indexOf("(")) {
        throw new Error("Got ')' before '(' ");
    }
    
    const openingParenthesesCount = strCount(equationStr, '(');
    const closingParenthesesCount = strCount(equationStr, ')');
    ...
    console.log(`-- DEBUG -- equationStr L: ${equationStr} --`)

    console.log(`tokens.join(', '): ${tokens.join(', ')}`)
...
export { calc, getFormattedEquationStr, MATH_FUNC_NAMES };
